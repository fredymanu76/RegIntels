...
SELECT
  ...
  CASE
    WHEN LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%process%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%procedure%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%workflow%'
      THEN 'Process'
    WHEN LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%training%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%staff%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%employee%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%human error%'
      THEN 'People'
    WHEN LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%system%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%technology%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%software%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%technical%'
      THEN 'Systems'
    WHEN LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%vendor%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%third party%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%supplier%'
      OR LOWER(COALESCE(e.description, '') || ' ' || COALESCE(e.remediation_plan, '')) LIKE '%outsourc%'
      THEN 'Third Party'
    ELSE 'Other'
  END as root_cause,
...
FROM exceptions e
LEFT JOIN controls cl ON cl.id = e.control_id
...
WITH monthly_exceptions AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE e.status IN ('open', 'in_remediation')) as open_count,
...
SELECT
  ...
  open_count,
  resolved_count,
  ROUND(avg_materiality::numeric, 1) as avg_materiality,
...
WITH current_exceptions AS (
  SELECT
    tenant_id,
    COUNT(*) FILTER (WHERE status IN ('open', 'in_remediation')) as open_exceptions,
    COUNT(*) FILTER (WHERE status = 'resolved' AND resolved_at >= CURRENT_DATE - INTERVAL '30 days') as resolved_last_30d,
    COUNT(*) FILTER (WHERE materiality_band = 'CRITICAL' AND status IN ('open', 'in_remediation')) as critical_open,
    COUNT(*) FILTER (WHERE materiality_band = 'HIGH' AND status IN ('open', 'in_remediation')) as high_open,
    COUNT(*) FILTER (WHERE materiality_band = 'MEDIUM' AND status IN ('open', 'in_remediation')) as medium_open,
    AVG(materiality_score) FILTER (WHERE status IN ('open', 'in_remediation')) as avg_open_materiality,
    AVG(days_open) FILTER (WHERE status IN ('open', 'in_remediation')) as avg_days_open
...
root_cause_summary AS (
  SELECT
    tenant_id,
    COUNT(*) FILTER (WHERE root_cause = 'Process' AND status IN ('open', 'in_remediation')) as process_exceptions,
    COUNT(*) FILTER (WHERE root_cause = 'People' AND status IN ('open', 'in_remediation')) as people_exceptions,
    COUNT(*) FILTER (WHERE root_cause = 'Systems' AND status IN ('open', 'in_remediation')) as systems_exceptions,
    COUNT(*) FILTER (WHERE root_cause = 'Third Party' AND status IN ('open', 'in_remediation')) as third_party_exceptions
