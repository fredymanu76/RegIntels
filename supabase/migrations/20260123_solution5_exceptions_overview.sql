-- ============================================================================
-- SOLUTION 5 - EXCEPTIONS OVERVIEW (BOARD VIEW) - ACTUAL SCHEMA
-- ============================================================================
-- Based on actual production schema (no tenant_id in exceptions!)
-- Date: 2026-01-23
-- ============================================================================
...
COMMENT ON VIEW v_exception_root_cause_taxonomy IS 'Solution 5: Root cause classification';
...
WITH monthly_exceptions AS (
  SELECT
    DATE_TRUNC('month', e.opened_at) as month,
    e.severity,
  ...
  GROUP BY DATE_TRUNC('month', e.opened_at), e.severity
),
trend_calculation AS (
  SELECT
    ...
    open_count,
    resolved_count,
    LAG(exception_count, 1) OVER (PARTITION BY severity ORDER BY month) as prev_month_count
...
CROSS JOIN root_cause_summary rcs
