-- ============================================================================
-- FINAL FIX - DROP AND RECREATE VIEWS (Fixed EXTRACT issue)
-- ============================================================================
-- This fixes the "function pg_catalog.extract does not exist" error
-- ============================================================================
...
SELECT
  ...
  CASE
    WHEN ra.due_date IS NOT NULL AND ra.due_date < CURRENT_DATE AND ra.status != 'completed'
...
SELECT
  ...
  NOW() - COALESCE(c.last_run_at, c.created_at) as review_delay_interval,
  EXTRACT(day FROM (NOW() - COALESCE(c.last_run_at, c.created_at)))::integer as review_delay_days,

  COALESCE(c.next_due_at, CURRENT_DATE + INTERVAL '999 days') - NOW() as time_to_next_review,
  EXTRACT(day FROM (COALESCE(c.next_due_at, CURRENT_DATE + INTERVAL '999 days') - NOW()))::integer as days_to_next_review,
...
-- ============================================================================
-- PART 4: ATTESTATION CONFIDENCE VIEW (FIXED EXTRACT ISSUES)
-- ============================================================================
...
SELECT
  ...
  CASE
    ...
    WHEN cr.performed_at::date <= cr.due_date + 7 THEN 25
    WHEN cr.performed_at::date <= cr.due_date + 14 THEN 15
    ELSE 5
  END as timeliness_score,

  -- Days late/early (FIXED: using simple date subtraction)
  CASE
    WHEN cr.performed_at IS NOT NULL
    THEN (cr.performed_at::date - cr.due_date)
    ELSE (NOW()::date - cr.due_date)
  END as days_delta,
  ...
  CASE
    ...
    WHEN cr.performed_at::date > cr.due_date + 7 THEN 'Late submission'
  ...
  (SELECT COUNT(*)
   FROM control_runs prev
   WHERE prev.performed_by = cr.performed_by
     AND prev.performed_at IS NOT NULL
...
-- ============================================================================
-- SUCCESS! All views created with EXTRACT fixes
-- ============================================================================
-- Test with:
