-- ============================================================================
-- EXCEPTIONS OVERVIEW - FINAL VERSION FOR ACTUAL SCHEMA
-- ============================================================================
-- Based on ACTUAL database schema:
-- - exceptions: source_id, source_type, opened_at, status, severity
-- - controls: id, title (not control_title)
-- ============================================================================
...
SELECT
  ...
  CASE
    WHEN e.source_type = 'control' THEN COALESCE(c.title, 'Unknown Control')
  ...
  -- Regulatory sensitivity (simplified - based on severity)
  CASE e.severity
...
SELECT
  ...
  e.source_type,
  CASE
    WHEN e.source_type = 'control' THEN COALESCE(c.title, 'Unknown')
    ELSE e.source_type
  END as source_name,
  e.opened_at,
  COALESCE((CURRENT_DATE - e.opened_at::date), 0) as days_open,
...
SELECT 'Success! Created 4 views for Exceptions Overview' as status;
...
-- ============================================================================
-- SUCCESS! Now navigate to Solution 5 → Exceptions Overview in your app
-- ============================================================================
