-- ============================================================================
-- REGINTELS STRATEGIC VIEWS - ADAPTED FOR EXISTING SCHEMA
-- ============================================================================
-- This creates strategic scoring views using your EXISTING tables:
-- - regulatory_changes (existing)
-- - control_library (existing)
-- - control_runs (existing - used as attestations)
-- - exceptions (existing)
-- - remediation_actions (existing - used as actions)
-- - regulatory_change_control_map (existing)
--
-- NO NEW TABLES CREATED - ONLY VIEWS!
-- ============================================================================

-- ============================================================================
-- PART 1: HELPER VIEW - Action Tracker
-- ============================================================================
-- Maps remediation_actions to change actions for scoring
...
SELECT
  ...
  -- Controls that need review but haven't been reviewed
  CASE
  ...
  -- Controls with failed or pending runs
  CASE
...
Adapted to use existing schema: regulatory_changes, control_library, control_runs, remediation_actions.';
...
Adapted to use existing schema: control_library, control_runs, exceptions.';
...
-- ============================================================================
-- PART 4: ATTESTATION CONFIDENCE VIEW (using control_runs)
-- ============================================================================
...
SELECT
  ...
  CASE
    WHEN cr.performed_at IS NULL AND NOW() > cr.due_date THEN 0  -- Overdue
    WHEN cr.performed_at IS NULL THEN 30  -- Pending but not overdue
    WHEN cr.performed_at::date <= cr.due_date THEN 40  -- On time
    WHEN cr.performed_at::date <= cr.due_date + INTERVAL '7 days' THEN 25  -- Slightly late
    WHEN cr.performed_at::date <= cr.due_date + INTERVAL '14 days' THEN 15  -- Late
    ELSE 5  -- Very late
...
Adapted to use existing schema: control_runs as attestations.';
...
-- MIGRATION COMPLETE!
-- ============================================================================
-- All strategic views have been created using your EXISTING tables.
-- No new tables were created - only views that analyze your current data.
--
-- Test with:
