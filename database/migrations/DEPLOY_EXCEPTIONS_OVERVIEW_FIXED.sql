-- ============================================================================
-- EXCEPTIONS OVERVIEW DEPLOYMENT - FIXED VERSION
-- ============================================================================
-- Solution 5 Enhancement - Module 1: Exceptions Overview
-- Strategic Purpose: Exception intelligence with materiality scoring,
-- trend analysis, and root cause taxonomy
--
-- Creates 3 new views building on existing v_exception_materiality
-- ============================================================================
...
-- - Materiality score (from existing v_exception_materiality)
-- - Recurrence patterns
...
SELECT
  ...
  -- Recurrence analysis (using direct table reference, not em alias)
  COALESCE(
    (SELECT COUNT(*)
     FROM exceptions e2
  ...
  CASE
    WHEN (SELECT COUNT(*) FROM exceptions ex WHERE ex.control_id = em.control_id AND ex.id != em.exception_id AND ex.created_at > CURRENT_DATE - INTERVAL '3 months') >= 3 THEN 'FREQUENT'
    WHEN (SELECT COUNT(*) FROM exceptions ex WHERE ex.control_id = em.control_id AND ex.id != em.exception_id AND ex.created_at > CURRENT_DATE - INTERVAL '12 months') >= 3 THEN 'RECURRING'
    WHEN (SELECT COUNT(*) FROM exceptions ex WHERE ex.control_id = em.control_id AND ex.id != em.exception_id) > 1 THEN 'OCCASIONAL'
    ELSE 'ISOLATED'
...
SELECT
  ...
  ROUND(COALESCE(avg_score_current_30d, 0)::numeric, 1) as avg_score_current_30d,
  ROUND(COALESCE(avg_score_previous_30d, 0)::numeric, 1) as avg_score_previous_30d,
  ROUND(COALESCE(avg_score_60d, 0)::numeric, 1) as avg_score_60d,
  ROUND(COALESCE(avg_score_90d, 0)::numeric, 1) as avg_score_90d,
...
SELECT
  ...
  CASE
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%process%' OR LOWER(COALESCE(e.description, '')) LIKE '%procedure%' OR LOWER(COALESCE(e.description, '')) LIKE '%workflow%' THEN 'PROCESS'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%training%' OR LOWER(COALESCE(e.description, '')) LIKE '%staff%' OR LOWER(COALESCE(e.description, '')) LIKE '%employee%' OR LOWER(COALESCE(e.description, '')) LIKE '%personnel%' THEN 'PEOPLE'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%system%' OR LOWER(COALESCE(e.description, '')) LIKE '%application%' OR LOWER(COALESCE(e.description, '')) LIKE '%software%' OR LOWER(COALESCE(e.description, '')) LIKE '%technical%' THEN 'SYSTEMS'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%vendor%' OR LOWER(COALESCE(e.description, '')) LIKE '%third party%' OR LOWER(COALESCE(e.description, '')) LIKE '%supplier%' OR LOWER(COALESCE(e.description, '')) LIKE '%outsource%' THEN 'THIRD_PARTY'
    ELSE 'UNCLASSIFIED'
  END as root_cause_category,

  -- Root cause subcategory
  CASE
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%policy%' OR LOWER(COALESCE(e.description, '')) LIKE '%governance%' THEN 'Policy/Governance Gap'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%control design%' OR LOWER(COALESCE(e.description, '')) LIKE '%design flaw%' THEN 'Control Design Issue'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%execution%' OR LOWER(COALESCE(e.description, '')) LIKE '%implementation%' THEN 'Execution Failure'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%capacity%' OR LOWER(COALESCE(e.description, '')) LIKE '%resource%' THEN 'Resource/Capacity Constraint'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%data quality%' OR LOWER(COALESCE(e.description, '')) LIKE '%data integrity%' THEN 'Data Quality Issue'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%technology%' OR LOWER(COALESCE(e.description, '')) LIKE '%infrastructure%' THEN 'Technology/Infrastructure'
    ELSE 'Other'
  ...
  CASE
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%process%' OR LOWER(COALESCE(e.description, '')) LIKE '%procedure%' THEN 'MODERATE'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%training%' OR LOWER(COALESCE(e.description, '')) LIKE '%staff%' THEN 'LOW'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%system%' OR LOWER(COALESCE(e.description, '')) LIKE '%software%' THEN 'HIGH'
    WHEN LOWER(COALESCE(e.description, '')) LIKE '%vendor%' OR LOWER(COALESCE(e.description, '')) LIKE '%third party%' THEN 'HIGH'
    ELSE 'MODERATE'
...
-- ============================================================================
-- VIEW 4: Summary View for Dashboard KPIs
-- ============================================================================
...
-- Next: Navigate to Solution 5 → Exceptions Overview in your app
-- ============================================================================
